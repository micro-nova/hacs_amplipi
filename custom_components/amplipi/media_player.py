...
from .const import (
    DOMAIN, AMPLIPI_OBJECT, CONF_VENDOR, CONF_VERSION, CONF_WEBAPP, COORDINATOR )
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    data = await data_coordinator._async_update_data()
    status = data.status
...
class AmpliPiMediaPlayer(MediaPlayerEntity, CoordinatorEntity):
    ...
    def get_entry_by_value(self, value: str) -> Union[Source, Zone, Group, Stream, None]:
        ...
        if self.coordinator.data is not None:
            ...
            for category in (status.sources, status.zones, status.groups, status.streams):
    ...
    def available_streams(self, source: Source):
        ...
        if self.coordinator.data is not None:
            ...
            stream_entries = self.coordinator.data.status.streams
            if stream_entries:
    ...
    async def async_connect_stream_to_source(self, stream: Stream, source: Optional[Source] = None):
        ...
        if stream.type == "rca":
            ...
            source = self.coordinator.data.status.sources[stream.id - 996]
    ...
    async def async_select_source(self, source):
        if self._source is not None and self._source.name == source:
            await self._client.set_source(
                self._id,
                SourceUpdate(
                    input=f"stream={996+self._source.id}"
                )
            )
        elif source == 'None':
            await self._client.set_source(
